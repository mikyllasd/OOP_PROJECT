...
import game.entities.BallType;
import game.entities.Basket;
import game.entities.BasketSkin;
import game.entities.Character;
import game.entities.FruitBall;
import game.entities.Particle;
import game.entities.PowerUp;
import game.entities.SkinType;
...
public class GamePanel extends JPanel implements Runnable, KeyListener, MouseMotionListener, MouseListener {
    ...
    private Map<SkinType, Character> skinPreviews = new HashMap<>();
    ...
    public void onResize(int newW, int newH) {
        ...
        if (character != null) {
            character.setX(ARENA_W / 2f - 25);
            character.setY(H - 160);
        }
    }
    ...
    // â”€â”€ game logic (unchanged) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private void gameUpdate() {
        ...
        if (magnetActive && magnetTimer > 0) {
            ...
            for (Ball b : balls) {
                ...
                if (dist < 200) b.translate((cx - bx2) * 0.06f, 0);
            }
        }
        ...
        if (tickCount % 1800 == 0 && tickCount > 0) {
            level++;
            sound.playLevelUp();
            showToast("ðŸŽ‰ Level Up! Level " + level);
    ...
    private void handleCatch(Ball b) {
        ...
        if (isBad) {
            if (shieldActive) {
                shieldActive = false;
                showToast("ðŸ›¡ï¸ Shield blocked it!");
    ...
    private void activatePowerUp(PowerUp.PowerUpType type) {
        sound.playPowerUp();
        switch (type) {
            case MAGNET:        magnetActive = true; magnetTimer = 300; showToast("ðŸ§² Magnet activated!"); break;
            case TIME_PLUS:     timeLeft += 15; showToast("â° +15 seconds!"); break;
            case SHIELD:        shieldActive = true; showToast("ðŸ›¡ï¸ Shield ready!"); break;
            case DOUBLE_POINTS: doublePointsActive = true; doubleTimer = 480; showToast("2ï¸âƒ£ Double Points!"); break;
        }
    }
    ...
    // â”€â”€ PAINTING â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Sky background â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private void drawSidebar(Graphics2D g) {
        ...
        drawSidebarItem(g, sx + 12, py, "COINS ðŸ’°", "" + playerData.getTotalCoins(), new Color(255, 215, 50), titleFont, new Font("Arial", Font.BOLD, 18)); py += 52;
        ...
        if (shieldActive)       { g.setFont(emoji20); g.setColor(Color.WHITE); g.drawString("ðŸ›¡ï¸", pip, py); pip += 30; }
        if (magnetActive)       { g.setFont(emoji20); g.setColor(Color.WHITE); g.drawString("ðŸ§²", pip, py); pip += 30; }
        if (doublePointsActive) { g.setFont(emoji20); g.setColor(Color.WHITE); g.drawString("2ï¸âƒ£", pip, py); pip += 30; }
        py += 30;
        ...
        drawSidebarActionButton(g, gameButtons[0], "ðŸ¦ Bank", gameHoveredButton == 0);
        drawSidebarActionButton(g, gameButtons[1], "ðŸ›’ Shop", gameHoveredButton == 1);
        drawSidebarActionButton(g, gameButtons[2], "âŒ Quit",   gameHoveredButton == 2);
        py += 92;
        ...
        drawIconButton(g, sx + 15, py, sound.isMuted() ? "ðŸ”‡" : "ðŸ”Š", "M", 60, 32);
        drawIconButton(g, sx + 85, py, "â¸", "P", 60, 32);
    }
    ...
    private void drawMainMenu(Graphics2D g) {
        ...
        String title = "ðŸŒ¾ Catch the Ball! ðŸšœ";
        FontMetrics fm = g.getFontMetrics();
        ...
        String[] labels = hasFarmer ? new String[]{"â–¶  Play Game", "ðŸ†  Leaderboard", "ðŸ§º  Wardrobe", "ðŸ¡  Farm Upgrade", "ðŸ…  Achievements", "âœï¸  Edit Farmer"} : new String[]{"â–¶  Play Game", "ðŸ†  Leaderboard", "ðŸ§º  Wardrobe", "ðŸ¡  Farm Upgrade", "ðŸ…  Achievements"};
        ...
        g.drawString("ðŸª™ " + playerData.getTotalCoins() + " coins", 20, H - 14);
    ...
    private void drawMenuButton(Graphics2D g, Rectangle r, String label, boolean hovered) {
        ...
        // Label â€” use emoji-capable font so icons render properly
    ...
    // â”€â”€ Character creation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private void drawCharCreation(Graphics2D g) {
        ...
        String t = "ðŸŒ¾ Create Your Farmer";
        FontMetrics fm = g.getFontMetrics();
        ...
        for (int i = 0; i < STARTER_SKINS.length; i++) {
            ...
            int previewBY = 348; // feet â€” lower so wizard hat doesn't get cut
            ...
            prev.setX(previewCX - prev.getWidth() / 2f);
            prev.setY(previewBY - prev.getHeight());
            prev.draw(g2c);
            g2c.dispose();
        }
        ...
        String btnText = "Start Farming! ðŸŒ¾";
        FontMetrics bfm = g.getFontMetrics();
        g.drawString(btnText, W/2 - bfm.stringWidth(btnText) / 2, 521);

        g.setFont(new Font("Arial", Font.BOLD, 13));
        g.setColor(new Color(180, 220, 150));
        g.drawString("â† Back to Menu", W/2 - 280 + 15, 553);
    }

    // â”€â”€ Edit Farmer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private void drawEditFarmer(Graphics2D g) {
        ...
        g.setFont(new Font("Arial Black", Font.BOLD, 26));
        g.setColor(new Color(255, 230, 80));
        String t = "âœï¸ Edit Your Farmer";
        FontMetrics fm = g.getFontMetrics();
        g.drawString(t, (W - fm.stringWidth(t)) / 2, 113);
        ...
        for (int i = 0; i < editableSkins.length; i++) {
            ...
            prev2.setX(previewCX - prev2.getWidth() / 2f);
            prev2.setY(previewBY - prev2.getHeight());
            prev2.draw(g2e);
            g2e.dispose();
        ...
        String btnText = "Save Changes ðŸŒ¾";
        FontMetrics bfm = g.getFontMetrics();
        g.drawString(btnText, W/2 - bfm.stringWidth(btnText) / 2, 521);
    ...
    private void drawPause(Graphics2D g) {
        ...
        g.drawString("â¸ PAUSED", px + 100, py + 55);
        ...
        drawMenuButton(g, menuButtons[0], "â–¶ Resume", hoveredButton == 0);
        drawMenuButton(g, menuButtons[1], "ðŸ  Menu",  hoveredButton == 1);

        g.setFont(new Font("Arial", Font.PLAIN, 12));
        g.setColor(new Color(160, 200, 140));
        g.drawString("Press P to resume â€¢ ESC for menu", px + 68, py + 250);
    }

    // â”€â”€ Wardrobe / Shop â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private void drawWardrobe(Graphics2D g) {
        ...
        drawScreenTitle(g, "ðŸ§º Wardrobe & Shop");

        // Tabs
        String[] tabs = {"ðŸ‘¨â€ðŸŒ¾ Farmer Skins", "ðŸ§º Basket Skins"};
        for (int i = 0; i < 2; i++) {
        ...
        g.setFont(new Font("Arial", Font.BOLD, 14));
        g.setColor(new Color(255, 215, 50));
        g.drawString("ðŸª™ " + playerData.getTotalCoins() + " coins", W - 200, 100);
        ...
        if (shopTab == 0) {
            ...
            // â”€â”€ FIX: basket skins drawn via skin.draw(), not getEmoji() â”€â”€
    ...
    private void drawShopItemEmoji(Graphics2D g, int x, int y, int w, int h,
        ...
        float scale = 0.58f;
        int previewCX = x + w / 2;
        int previewBY = y + 85; // feet position â€” lower so full body fits
        ...
        float oldX = preview.getX(), oldY = preview.getY();
        preview.setX(previewCX - preview.getWidth() / 2f);
        preview.setY(previewBY - preview.getHeight());
        preview.draw(g2);
        preview.setX(oldX);
        preview.setY(oldY);
        g2.dispose();
    ...
    /** Shop card for BasketSkin (drawn via skin.draw()) â€” FIX for getEmoji() error */
    ...
    private void drawShopCardLabels(Graphics2D g, int x, int y, int w, int h,
        ...
        if (equipped) {
            ...
            g.drawString("âœ“ EQUIPPED", x + (w - efm.stringWidth("âœ“ EQUIPPED")) / 2, y + 100);
        } else if (owned) {
            g.setColor(new Color(100, 205, 80));
            g.setFont(new Font("Arial", Font.BOLD, 11));
            FontMetrics ofm = g.getFontMetrics();
            g.drawString("OWNED â€” Equip", x + (w - ofm.stringWidth("OWNED â€” Equip")) / 2, y + 100);
        } else {
            ...
            String costStr = "ðŸª™ " + cost;
            FontMetrics fm3 = g.getFontMetrics();
            g.drawString(costStr, x + 10 + (w - 20 - fm3.stringWidth(costStr)) / 2, y + 108);
        }
    }

    // â”€â”€ Farm upgrade â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    private void drawFarmUpgrade(Graphics2D g) {
        ...
        for (int i = 0; i < H; i += 20) g.drawLine(0, i, W, i);
        drawScreenTitle(g, "ðŸ¡ Farm Upgrade");
        ...
        g.drawString("ðŸª™ Coins: " + playerData.getTotalCoins(), W/2 - 80, 308);
        ...
        if (farm.canUpgrade()) {
            ...
            g.setColor(Color.WHITE);
            String upgStr = "Upgrade for ðŸª™ " + farm.getUpgradeCost();
            FontMetrics fm3 = g.getFontMetrics();
            g.drawString(upgStr, W/2 - fm3.stringWidth(upgStr) / 2, 401);
        } else {
            g.setFont(new Font("Arial Black", Font.BOLD, 22));
            g.setColor(new Color(255, 215, 50));
            g.drawString("ðŸ‘‘ MAX LEVEL FARM!", W/2 - 140, 378);
        }
    ...
    private void drawAchievements(Graphics2D g) {
        ...
        drawScreenTitle(g, "ðŸ… Achievements");
        ...
        for (int i = 0; i < all.size(); i++) {
            ...
            if (unlocked) {
                ...
                g.drawString("âœ“ UNLOCKED", bx + 52, by + 76);
            }
        }
        drawBackButton(g);
    }
    ...
    private void drawGameOver(Graphics2D g) {
        ...
        drawStatLine(g, px + 30, statY, "Coins Earned",  "+" + coinsThisGame + " ðŸª™"); statY += 34;
        ...
        drawMenuButton(g, menuButtons[0], "ðŸ”„ Retry",     hoveredButton == 0);
        drawMenuButton(g, menuButtons[1], "ðŸ† Save Score", hoveredButton == 1);
        drawMenuButton(g, menuButtons[2], "ðŸ  Menu",       hoveredButton == 2);
    }
    ...
    private void drawLeaderboard(Graphics2D g) {
        ...
        drawScreenTitle(g, "ðŸ† Leaderboard");

        List<ScoreEntry> top = scoreManager.getTop10();
        String[] ranks = {"ðŸ‘‘","ðŸ¥ˆ","ðŸ¥‰","4ï¸âƒ£","5ï¸âƒ£","6ï¸âƒ£","7ï¸âƒ£","8ï¸âƒ£","9ï¸âƒ£","ðŸ”Ÿ"};
    ...
    private void drawBackButton(Graphics2D g) {
        ...
        drawMenuButton(g, menuButtons[0], "â† Back", hoveredButton == 0);
    }
            ...
            case FARM_UPGRADE:
                if (new Rectangle(W/2 - 150, 370, 300, 52).contains(mx, my) && farm.canUpgrade()) {
                    ...
                    if (playerData.spendCoins(cost)) {
                        ...
                        showToast("ðŸ¡ Farm upgraded to " + farm.getName() + "!");
                        checkAchievements();
                        playerData.setAchievementData(achievements.saveToString());
                    } else {
                        showToast("Not enough coins! Need ðŸª™ " + cost);
                    }
                }
                if (menuButtons != null && menuButtons.length > 0 && menuButtons[0] != null && menuButtons[0].contains(mx, my)) screen = GameScreen.MAIN_MENU;
                break;
                ...
                if (mx > ARENA_W) {
                    if (gameButtons != null) {
                        if (gameButtons.length > 0 && gameButtons[0] != null && gameButtons[0].contains(mx, my)) {
                            showToast("ðŸª™ Wallet: " + playerData.getTotalCoins() + " coins");
            ...
            for (int i = 0; i < skins.length; i++) {
                ...
                if (new Rectangle(bx, by, boxW, boxH).contains(mx, my)) {
                    if (playerData.ownsSkin(skins[i])) {
                        ...
                        showToast("Not enough coins! Need ðŸª™ " + skins[i].getCost());
                    }
                }
            }
        } else {
            ...
            for (int i = 0; i < baskets.length; i++) {
                ...
                if (new Rectangle(bx, by, boxW, boxH).contains(mx, my)) {
                    if (playerData.ownsBasket(baskets[i])) {
                        ...
                        showToast("Not enough coins! Need ðŸª™ " + baskets[i].getCost());
                    }
                }
            }
        }
