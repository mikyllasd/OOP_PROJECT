...
public class FontManager {
    ...
    public static Font getEmoji(int size) {
        if (availableFonts.contains(EMOJI_PRIMARY))   return new Font(EMOJI_PRIMARY,   Font.PLAIN, size);
        if (availableFonts.contains(EMOJI_FALLBACK1)) return new Font(EMOJI_FALLBACK1, Font.PLAIN, size);
        if (availableFonts.contains(EMOJI_FALLBACK2)) return new Font(EMOJI_FALLBACK2, Font.PLAIN, size);
        return new Font(Font.SANS_SERIF, Font.PLAIN, size);
    }

    public static Font getBold(int size) {
        if (availableFonts.contains(BOLD_PRIMARY))  return new Font(BOLD_PRIMARY,  Font.PLAIN, size);
        if (availableFonts.contains(BOLD_FALLBACK)) return new Font(BOLD_FALLBACK, Font.BOLD,  size);
        return new Font(Font.SANS_SERIF, Font.BOLD, size);
    }

    public static Font getBody(int size, int style) {
        if (availableFonts.contains(BODY_PRIMARY)) return new Font(BODY_PRIMARY, style, size);
