...
public class RenderUtils {
    ...
    public static void drawRoundPanel(Graphics2D g, int x, int y, int w, int h,
    ...
    public static void drawGradientPanel(Graphics2D g, int x, int y, int w, int h,
            Color top, Color bottom, Color border, float borderWidth, int arc) {
    ...
    public static void drawButton(Graphics2D g, Rectangle r, String label,
    ...
    public static void drawCenteredText(Graphics2D g, String text,
            int cx, int y, Font font, Color color) {
        g.setFont(font);
        g.setColor(color);
        FontMetrics fm = g.getFontMetrics();
    ...
    public static void drawShadowText(Graphics2D g, String text, int x, int y,
    ...
    public static void drawProgressBar(Graphics2D g, int x, int y, int w, int h,
            float progress, Color bg, Color fillStart, Color fillEnd) {
        g.setColor(bg);
        g.fillRoundRect(x, y, w, h, h, h);
        int fillW = (int)(w * Math.min(1f, progress));
        if (fillW > 0) {
            GradientPaint gp = new GradientPaint(x, y, fillStart, x + fillW, y, fillEnd);
            g.setPaint(gp);
    ...
    public static void drawHeaderBar(Graphics2D g, int w, String title) {
        GradientPaint hdr = new GradientPaint(0, 0, new Color(45,100,30,210), 0, 75, new Color(28,65,18,210));
        g.setPaint(hdr);
        g.fillRect(0, 0, w, 75);
        g.setPaint(null);
        ...
        g.setStroke(new BasicStroke(1f));
        drawCenteredText(g, title, w / 2, 50, FontManager.getBold(26), ColorPalette.TEXT_GOLD);
    }
